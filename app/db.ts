...
export const votesTable = pgTable(
  ...
  (t) => ({
    // for now, we only need to query by user_id, but we
    // can use a compound key to also ensure uniqueness
    story_id_user_id_idx: uniqueIndex("story_id_user_id_idx").on(
      t.story_id,
      t.user_id
    ),
  })
